    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                ScrollView {
                    VStack(spacing: 8) {
                        ForEach(pinpoints.filter { $0.building.localizedCaseInsensitiveContains(searchText) }, id: \.id) { item in
                            Button(action: {
                                self.navigate = true
...
struct BuildingPage_Previews: PreviewProvider {
    static var previews: some View {
        BuildingPage(searchText:"C Tower").environmentObject(GlobalData())
    }
}
